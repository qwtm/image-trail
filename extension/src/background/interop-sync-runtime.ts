...
export class InteropSyncRuntime {
  ...
  async resume(provider: InteropProviderId, sessionId: string): Promise<SecureSyncProgress> {
    ...
    return new SyncOutboxPublisher(db, store).resume(sessionId, pairing);
  }

  async status(sessionId: string): Promise<SecureSyncProgress | null> {
    const db = await this.getDb();
    return db ? new SecureSyncOutboxRepository(db).progress(sessionId) : null;
  }
