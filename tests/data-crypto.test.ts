...
test('seals and opens a versioned AES-GCM JSON envelope with authenticated metadata', async () => {
  ...
  const payload = { url: 'https://example.test/001.jpg', capturedAt: '2026-06-16T00:00:00.000Z', captureStatus: 'remote-only' as const };
  const envelope = await sealJsonEnvelope({ payload, payloadVersion: 1, key: session.key, keyReference: session.reference, authenticatedMetadata: { recordType: 'history' }, now: '2026-06-16T00:00:00.000Z' });
...
test('loads typed plaintext local settings through defaults and migrations', () => {
  ...
  const repository = new LocalSettingsRepository({ getItem: (key) => values.get(key) ?? null, setItem: (key, value) => { values.set(key, value); } });
